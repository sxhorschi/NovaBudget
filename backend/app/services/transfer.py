...
def _clone_cost_item(
    ...
    original_amount = Decimal("0") if reset_amounts else item.original_amount
    current_amount = Decimal("0") if reset_amounts else item.current_amount
...
async def transfer_cost_items(
    ...
    for item in source_items:
        if mode == "copy":
            ...
            if reset_amounts:
                item.original_amount = Decimal("0")
                item.current_amount = Decimal("0")
...
async def transfer_work_areas(
    ...
    Move mode: re-parent work areas to target department.
    """
    if mode not in ("copy", "move"):
        raise ValueError(f"Invalid transfer mode: {mode!r}. Must be 'copy' or 'move'.")

    # Validate target department exists
    target_dept = await session.get(Department, target_department_id)
    if not target_dept:
        raise ValueError(f"Target department {target_department_id} not found")
    ...
    for wa in source_was:
        if mode == "copy":
            ...
            old_id = wa.id
            wa.department_id = target_department_id
...
async def transfer_departments(
    session: AsyncSession,
    source_facility_id: uuid.UUID,
    target_facility_id: uuid.UUID,
    ...
    """Transfer departments (with work areas and cost items) between facilities.

    Copy mode: duplicate entire department hierarchy with new UUIDs.
    Move mode: re-parent departments to target facility.
    """
    if mode not in ("copy", "move"):
        raise ValueError(f"Invalid transfer mode: {mode!r}. Must be 'copy' or 'move'.")
    ...
    # Load source departments with full hierarchy
    stmt = (
        select(Department)
    ...
    result = await session.execute(stmt)
    source_depts = result.scalars().all()

    if len(source_depts) != len(department_ids):
        found_ids = {d.id for d in source_depts}
        missing = set(department_ids) - found_ids
        raise ValueError(f"Departments not found: {missing}")

    transferred: list[Department] = []
    logs: list[TransferLog] = []
    ...
    for dept in source_depts:
        if mode == "copy":
            ...
            logs.append(_log_entry(
                "department", dept.id, new_dept.id,
                source_facility_id, target_facility_id, "copy", user,
            ))
            ...
            transferred.append(new_dept)
        else:  # move
            old_id = dept.id
            dept.facility_id = target_facility_id
            if reset_status or reset_amounts:
                for wa in dept.work_areas:
                    for item in wa.cost_items:
                        if reset_status:
                        ...
                        if reset_amounts:
                            ...
                            item.zielanpassung_reason = None
                        item.expected_cash_out = None
            transferred.append(dept)
            logs.append(_log_entry(
                "department", old_id, old_id,
                source_facility_id, target_facility_id, "move", user,
            ))
