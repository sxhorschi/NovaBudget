...
// ---------------------------------------------------------------------------
// FunctionalAreaRow — Top-level collapsible row with progress bar
// ---------------------------------------------------------------------------

interface FunctionalAreaRowProps {
  name: string;
  committed: number;
  budget: number;
  itemCount: number;
  ...
  onToggle: () => void;
  onOpenContext?: () => void;
  /** Functional Area accent color (hex) for left-border and progress bar */
  accentColor: string;
}

export default function FunctionalAreaRow({
  name,
  committed,
  budget,
  itemCount,
        ...
        <div className="inline-flex w-full items-center justify-end gap-2">
          ...
          {onOpenContext && (
            <button
              ...
              aria-label="Edit functional area"
            >
              <Pencil size={14} />
            </button>
          )}
